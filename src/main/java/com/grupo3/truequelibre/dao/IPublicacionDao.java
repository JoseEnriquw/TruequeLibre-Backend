...
public interface IPublicacionDao extends JpaRepository<Publicacion,Integer>{
	@Query(value="select * from publicacion where usuario_id = :usuario and estado_id != 2 and estado_id != 6",nativeQuery=true)
	List<Publicacion> findByUser(Integer usuario);
	
	@Query(value="select * from publicacion where categoria_id = :categoria and estado_id != 2 and estado_id != 6 and usuario_id != :usuario",nativeQuery=true)
